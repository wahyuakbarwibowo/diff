...
export function printHelp() {
  console.log(`devtools-offline\n\nUsage:\n  devtools-offline serve [--host 127.0.0.1] [--port 4173] [--open]\n  devtools-offline diff <left> <right>\n  devtools-offline json <format|minify|sort|validate> <input>\n  devtools-offline jwt decode <token>\n  devtools-offline timestamp [value]\n  devtools-offline hash [sha256|sha1|sha384|sha512|md5] <input> [--file]\n  devtools-offline case <camel|pascal|snake|kebab|constant|dot|path|title|lower|upper> <input>\n  devtools-offline url <inspect|encode|decode> <input>\n  devtools-offline uuid [v4|v7|ulid] [count]\n\nNotes:\n  - No command defaults to 'serve' for the browser UI.\n  - Use quotes for text with spaces.\n  - For file-based commands, pass --file before the path.\n`);
}
...
async function runDiff(args) {
  if (args.length < 2) throw new Error("Usage: devtools-offline diff <left> <right>");
...
async function runCase(args) {
  const target = args[0];
  if (!target) throw new Error("Usage: devtools-offline case <target> <input>");
  const input = await readSingleInput(args.slice(1));
